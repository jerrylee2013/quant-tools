...
export default function Page() {
  ...
  const calculateGrid = () => {
    ...
    // } else 
    if (kk === 100) {
      h = (2 * a * f * n * p0 * pb0 - 2 * a * f * n * p0 * ph - a * f * n * pb0 ** 2 + 2 * a * f * n * pb0 * ph + 2 * a * f * n * pb0 * ps0 - a * f * n * ph ** 2 - 2 * a * f * n * ph * ps0 - a * f * pb0 * ph + a * f * pb0 * pl + a * f * ph ** 2 - a * f * ph * pl + 2 * a * n * p0 * pb0 - 2 * a * n * p0 * ph + a * n * pb0 ** 2 - 2 * a * n * pb0 * ph - 2 * a * n * pb0 * ps0 + a * n * ph ** 2 + 2 * a * n * ph * ps0 + a * pb0 * ph - a * pb0 * pl - a * ph ** 2 + a * ph * pl) / (2 * f * p0 * ph - 2 * f * p0 * pl + 2 * f * ph ** 2 - 2 * f * ph * pl - 2 * p0 * ph + 2 * p0 * pl + 2 * ph ** 2 - 2 * ph * pl);
    } else {
      let k: number = kk / (100 - kk);
      h = (2 * a * f * k * n * p0 * pb0 - 2 * a * f * k * n * p0 * ph - a * f * k * n * pb0 ** 2 + 2 * a * f * k * n * pb0 * ph + 2 * a * f * k * n * pb0 * ps0 - a * f * k * n * ph ** 2 - 2 * a * f * k * n * ph * ps0 - a * f * k * pb0 * ph + a * f * k * pb0 * pl + a * f * k * ph ** 2 - a * f * k * ph * pl - 2 * a * f * n * p0 * pb0 + 2 * a * f * n * p0 * ph - a * f * n * pl ** 2 + a * f * n * ps0 ** 2 + a * f * ph * pl - a * f * ph * ps0 - a * f * pl ** 2 + a * f * pl * ps0 + 2 * a * k * n * p0 * pb0 - 2 * a * k * n * p0 * ph + a * k * n * pb0 ** 2 - 2 * a * k * n * pb0 * ph - 2 * a * k * n * pb0 * ps0 + a * k * n * ph ** 2 + 2 * a * k * n * ph * ps0 + a * k * pb0 * ph - a * k * pb0 * pl - a * k * ph ** 2 + a * k * ph * pl - 2 * a * n * p0 * pb0 + 2 * a * n * p0 * ph + 2 * a * n * pb0 * pl - 2 * a * n * ph * pl + a * n * pl ** 2 - 2 * a * n * pl * ps0 + a * n * ps0 ** 2 + a * ph * pl - a * ph * ps0 - a * pl ** 2 + a * pl * ps0) / (2 * f * k * p0 * ph - 2 * f * k * p0 * pl + 2 * f * k * ph ** 2 - 2 * f * k * ph * pl - 2 * f * p0 * ph + 2 * f * p0 * pl - 2 * f * ph * pl + 2 * f * pl ** 2 - 2 * k * p0 * ph + 2 * k * p0 * pl + 2 * k * ph ** 2 - 2 * k * ph * pl + 2 * p0 * ph - 2 * p0 * pl - 2 * ph * pl + 2 * pl ** 2)
    }

    console.log('h:', h);
    let s: number = (ph - pb0) / u;
    ...
    setGridResult({
      ...
      Vl: a * (s + b) * (ps0 - b * u),
      Dh: h * (ph - p0),
      Vh: a * (ps0 * s + s * (s - 1) * u / 2 + pl * b + b * (b - 1) * u / 2),
      Cdh: h * ph * f + a * (ps0 * s + s * (s - 1) * u / 2) * f
    });
  }
